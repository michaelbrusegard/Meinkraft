...
impl MeshGenerator {
    ...
    fn add_face(
        ...
        let bbl = [x - 0.5, y - 0.5, z - 0.5];
        let bbr = [x + 0.5, y - 0.5, z - 0.5];
        let btr = [x + 0.5, y + 0.5, z - 0.5];
        let btl = [x - 0.5, y + 0.5, z - 0.5];
        let fbl = [x - 0.5, y - 0.5, z + 0.5];
        let fbr = [x + 0.5, y - 0.5, z + 0.5];
        let ftr = [x + 0.5, y + 0.5, z + 0.5];
        let ftl = [x - 0.5, y + 0.5, z + 0.5];

        let uv_bl = [u_min, v_min];
        let uv_br = [u_max, v_min];
        let uv_tr = [u_max, v_max];
        let uv_tl = [u_min, v_max];

        let (face_verts, face_uvs): ([[f32; 3]; 4], [[f32; 2]; 4]) = match params.face_index {
            0 => ([bbr, fbr, ftr, btr], [uv_bl, uv_br, uv_tr, uv_tl]),
            1 => ([fbl, bbl, btl, ftl], [uv_bl, uv_br, uv_tr, uv_tl]),
            2 => ([btl, ftl, ftr, btr], [uv_bl, uv_br, uv_tr, uv_tl]),
            3 => ([bbl, fbl, fbr, bbr], [uv_bl, uv_br, uv_tr, uv_tl]),
            4 => ([fbl, fbr, ftr, ftl], [uv_bl, uv_br, uv_tr, uv_tl]),
            5 => ([bbr, bbl, btl, btr], [uv_bl, uv_br, uv_tr, uv_tl]),
            _ => unreachable!(),
        };
        for i in 0..4 {
            vertices.extend_from_slice(&face_verts[i]);
            vertices.extend_from_slice(&face_uvs[i]);
        }
